...
- [ ] 1.1 在 `OracleToMySQLConverter` 类中添加 COMMENT 存储字典
...
- [ ] 6.1 更新 `tools/oracle_to_mysql.py` 的 docstring
  - 添加 `--table-prefix` 参数说明
  - 添加 `--enable-comments` 参数说明
  - 添加 `--auto-fix` 参数说明
  - 添加使用示例

- [ ] 6.2 更新 `tools/sql-fix-tools/README.md`
  - 说明自动化集成功能
  - 添加 `--target-dir` 参数说明
  - 更新工作流程图
...
3.1 → 3.2 → 3.3 (表名前缀)
   ↓
4.1 → 4.2 → 4.3 → 4.4 (自动修复)
   ↓
5.1, 5.2, 5.3 (单元测试，可并行)
   ↓
5.4 (端到端测试)
   ↓
6.1, 6.2, 6.3 (文档更新，可并行)
   ↓
7.1, 7.2, 7.3 (最终验证)
```
