...
- `tools/oracle_to_mysql.py`: 
  ...
  - `convert_file` 函数需要集成自动修复调用
  - `main` 函数需要添加新的命令行参数
- `tools/sql-fix-tools/`: 需要提供可编程调用的接口
- `backend-django/core/management/commands/import_oracle_sql.py`: 可能需要更新以支持带前缀的表名验证
...
- 新功能通过可选参数启用
- 默认行为与当前版本一致（不添加 COMMENT，不添加表名前缀，不自动修复）
