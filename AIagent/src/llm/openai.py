...
class OpenAILLMClient(BaseLLMClient):
    ...
    def __init__(self, config: LLMConfig):
        ...
        if not config.api_key:
            raise ValueError("OpenAI需要设置api_key")
        
        self.api_key = config.api_key
        self.model = config.model
        self.temperature = config.temperature
        self.max_tokens = config.max_tokens
        self.base_url = config.base_url  # 支持自定义端点（如OpenAI兼容的API）
    
    def get_langchain_model(self) -> BaseLanguageModel:
        """
        获取LangChain ChatOpenAI模型实例
        ...
        if self._langchain_model is None:
            ...
            # 如果设置了base_url，使用自定义端点
            if self.base_url:
                model_kwargs['base_url'] = self.base_url
            
            self._langchain_model = ChatOpenAI(**model_kwargs)
    ...
    async def stream(
        ...
        **kwargs
        ...
        try:
            ...
            self.logger.error(f"OpenAI流式调用失败: {e}")
            raise
