...
import InvokePanel from './InvokePanel.vue';
    ...
    <ElDrawer :model-value="visible" title="接口详情" size="50%" direction="rtl" @close="handleClose">
        ...
        <template v-else-if="detail">
            <ElTabs v-model="activeTab">
                <!-- 接口信息 Tab -->
                <ElTabPane label="接口信息" name="info">
                    <ElCollapse v-model="activeNames">
                        ...
                        <!-- 请求参数 -->
                        <ElCollapseItem v-if="detail.parameters && detail.parameters.length > 0" title="请求参数"
                            name="parameters">
                            <div class="overflow-x-auto">
                                <table
                                    class="min-w-full border-collapse border border-gray-200 text-sm dark:border-gray-700">
                                    <thead>
                                        <tr class="bg-gray-50 dark:bg-gray-800">
                                            <th class="border border-gray-200 px-3 py-2 text-left dark:border-gray-700">
                                                名称</th>
                                            <th class="border border-gray-200 px-3 py-2 text-left dark:border-gray-700">
                                                位置</th>
                                            <th class="border border-gray-200 px-3 py-2 text-left dark:border-gray-700">
                        ...
                        <ElCollapseItem v-if="detail.responses && Object.keys(detail.responses).length > 0" title="响应定义"
                            ...
                            <div v-for="(response, statusCode) in detail.responses" :key="statusCode" class="mb-4">
                                <h4 class="mb-2 font-medium">
                                    <ElTag :type="String(statusCode).startsWith('2') ? 'success' : 'warning'"
                                        size="small">
                                        {{ statusCode }}
                                    </ElTag>
                        ...
                        <!-- 示例数据 -->
                        <ElCollapseItem v-if="detail.request_example || detail.response_example" title="示例数据"
                            name="examples">
                        ...
                        <!-- 额外信息 -->
                        <ElCollapseItem v-if="detail.request_fields || detail.response_fields || detail.location"
                            title="额外信息" name="extra">
                ...
                <!-- 调用测试 Tab（仅可访问接口显示） -->
                <ElTabPane v-if="accessible" label="调用测试" name="invoke">
                    <InvokePanel :operation-id="operationId" />
                </ElTabPane>

                <!-- 调用历史 Tab -->
                <ElTabPane label="调用历史" name="history">
                    <InvokeHistory :operation-id="operationId" />
                </ElTabPane>
            </ElTabs>
        </template>
